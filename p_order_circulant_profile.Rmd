...
## Kappa functions given other params

### Numerical
```{r}
score_beta_numerical_given_kappa <- function(beta_vec, V, mu, kappa_est) {
  N <- ncol(V)
  m <- nrow(V)
  p <- length(beta_vec)
  f <- numeric(p)
...
solve_params_numerical_given_kappa <- function(initial_guess_betas, V, kap) {
  mu_est <- mean(V)
  solution <- multiroot(f = function(bet) score_beta_numerical_given_kappa(bet, V, mu_est, kap), start = initial_guess_betas)
  beta_est_num <- solution$root
  
  return(
    list('beta_est' = beta_est_num,
         'mu_est' = mu_est)
  )
}
```
...
for (i in 1:n_vals) {
  kap = kappas[i]
  params_given_kappa = solve_params_numerical_given_kappa(initial_guess_betas = c(-0.25, -0.1), V, kap)
  betas = params_given_kappa$beta_est
  ll_kappas[i] <- log_like(c(betas[1], betas[2]), kap, mu_est, V)
}
...
legend(x = 'topleft', legend = c('True value', 'Estimate numerical', 'Estimate analytical'), col = c('black', 'red', 'blue'), lty = 'dashed', cex = 0.8, bty ='n')
```


## Other parameters given fixed beta 1
Fikser beta1, estimer beta2 og kappa givet denne
```{r}
score_beta2_numerical_given_beta1 <- function(beta_1_fixed, beta_2, V, mu) {
  beta_vec = c(beta_1_fixed, beta_2)
  N <- ncol(V)
  m <- nrow(V)
  p <- length(beta_vec)
  f <- numeric(p)
  ...
  return(f[2])  # only want the beta_2 part here
}

solve_beta2_given_beta1 <- function(beta1_fixed, V, mu) {
  
  f1 <- function(beta2) {
    score_beta2_numerical_given_beta1(beta1_fixed, beta2, V, mu)
  }
  root  <- uniroot(f1, interval = c(-0.15, -0.05))$root
  return(root)
}
```
...
for (i in 1:n_vals) {
  beta_1 = beta_1s[i]
  beta_2_given_beta_1 = solve_beta2_given_beta1(beta_1, V, mu_est)
  kappa_given_betas = kappa_hat_numerical(c(beta_1, beta_2_given_beta_1), mu_est, V)
  ll_beta_1s[i] <- log_like(c(beta_1,  beta_2_given_beta_1), kappa_given_betas, mu_est, V)
}
...
legend(x = 'topright', legend = c('True value', 'Estimate numerical', 'Estimate analytical'), col = c('black', 'red', 'blue'), lty = 'dashed', cex = 0.7, bty = 'n')
```



## Other parameters given fixed beta 2
Fikser beta2, estimer beta1 og kappa givet denne
```{r}
score_beta1_numerical_given_beta2 <- function(beta_1, beta_2_fixed, V, mu) {
  beta_vec = c(beta_1, beta_2_fixed)
  N <- ncol(V)
  m <- nrow(V)
  p <- length(beta_vec)
  f <- numeric(p)
  ...
  return(f[1])  # only want the beta_1 part here
}

solve_beta1_given_beta2 <- function(beta2_fixed, V, mu) {
  
  f1 <- function(beta1) {
    score_beta1_numerical_given_beta2(beta1, beta2_fixed, V, mu)
  }
  root  <- uniroot(f1, interval = c(-0.3, -0.2))$root
  return(root)
}
```
...
for (i in 1:n_vals) {
  beta_2 = beta_2s[i]
  beta_1_given_beta_2 = solve_beta1_given_beta2(beta_2, V, mu_est)
  kappa_given_betas = kappa_hat_numerical(c(beta_1_given_beta_2, beta_2), mu_est, V)
  ll_beta_2s[i] <- log_like(c(beta_1_given_beta_2,  beta_2), kappa_given_betas, mu_est, V)
}
